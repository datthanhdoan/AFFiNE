...
enum Flag {
  ...
  Hovering = 0b10000,
  // Dragging something, e.g. drag handle, drag resources from outside
  Dragging = 0b100000,
}
  ...
  override connectedCallback() {
    ...
    disposables.add(
      effect(() => {
        ...
        if (this.containsWith(Flag.Hovering | Flag.Dragging, value)) return;
    ...
    const dragStart = () => this.toggleWith(Flag.Dragging, true);
    const dragEnd = () => this.toggleWith(Flag.Dragging, false);
    ...
    disposables.add(
      flags$.subscribe(value => {
        ...
        // Hides toolbar
        if (this.checkWith(Flag.Dragging, value)) {
