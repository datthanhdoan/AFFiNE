    ...
    return {
      content: html`
        <editor-menu-button
          .contentPadding="${'8px'}"
          .button=${html`
            <editor-icon-button aria-label=${conversion.name} .tooltip="${''}">
...
const inlineTextActionGroup = {
  id: 'inline-text',
  when: ({ chain }) => isFormatSupported(chain).run()[0],
  actions: textFormatConfigs.map(({ id, name, action, activeWhen, icon }) => {
    return {
      id,
      icon,
      tooltip: name,
    ...
    return {
      content: html`
        <editor-menu-button
          .contentPadding="${'8px'}"
          .button=${html`
            <editor-icon-button aria-label="${''}" .tooltip="${''}">
